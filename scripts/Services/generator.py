...
def generate(primer = example, qpm = 120, num_steps = 120, 
  ...
  if max(distances) == 0:
    temperature += 0.1
    return generate(primer, temperature)

  randomFragment = random.randint(12)
  while getDistance(fragment(bins, randomFragment), primer_drums)) == 0:
    randomFragment = random.randint(12)
  ...
  return encondeFragment(fragment(bins, randomFragment))
